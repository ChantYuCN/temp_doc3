...
Options:
    ...
    --wheels-python2: Specify path to python2 based wheels tarball or image, URL or docker tag
...
function post_build {
    ...
    echo "Record python module and package -- start"
    if [ "${OS}" = "centos" ]; then
...
#chant test
    #local SERVICES_PYTHON2
    #SERVICES_PYTHON2=$(source /home/yuchengde/starlingx/docker-image-build.cfg && echo ${SERVICES_PYTHON2})
    #local WHEELS_PYTHON2
    #WHEELS_PYTHON2=$(source /home/yuchengde/starlingx/docker-image-build.cfg && echo ${WHEELS_PYTHON2})

    echo "chant echo service ${SERVICES_PYTHON2}"
    #echo "chant echo wheel ${WHEELS_PYTHON2}"
...
#chant test
    ...
    echo ${SERVICES_PYTHON2}
    
    OLD_IFS="$IFS"
    IFS=","
    SERVICES_PYTHON2_ARRAY=($SERVICES_PYTHON2)
    IFS="$OLD_IFS"
   
    echo ${SERVICES_PYTHON2_ARRAY[@]}


    echo ${WHEELS_PYTHON2}

    if is_in ${LABEL} ${SERVICES_PYTHON2_ARRAY[@]}; then
        echo "Python2 service ${LABEL}"
        BUILD_ARGS+=(--build-arg WHEELS=${WHEELS_PYTHON2})
    else
        echo "Python3 service ${LABEL}"
        BUILD_ARGS+=(--build-arg WHEELS=${WHEELS})
    fi

    echo "chant test label ${LABEL}"
...
OPTS=$(getopt -o h -l help,os:,version:,release:,stream:,push,proxy:,user:,registry:,base:,wheels:,python2-wheels:,only:,skip:,prefix:,latest,latest-prefix:,clean,attempts: -- "$@")
...
if [ ${#SERVICES_PYTHON2[@]} -ne 0 ] && [ -z "${WHEELS_PYTHON2}" ]; then
    echo "Path to wheels-python2 tarball must be specified with --wheels-python2 option." >&2
    exit 1
fi
...
#SERVICES_PYTHON2=(`source ${docker_image_build_cfg_file} && echo ${SERVICES_PYTHON2} | sed s/,/\ /g`) 
# Get python2 based services from cfg file
SERVICES_PYTHON2=(`source ${IMAGE_BUILD_CFG_FILE} && echo ${SERVICES_PYTHON2} | sed s/,/\ /g`)
echo "chant tery ${SERVICES_PYTHON2[2]}"

cat ${WORKDIR}/loci/bindep.txt
